...
- Приклад без помилки: виводить `alert` `(1)` та `(2)`:
    ...
    ```
- Приклад з помилкою: виводить `(1)` та `(3)`:
...
````warn header="`try...catch` працює тільки з помилками, що виникають під час виконання скрипту"
...
Тому `try...catch` може тільки обробляти помилки, що виникають у правильному коді. Такі помилки називаються "помилки під час виконання" (runtime errors) або "виключення" (exceptions).
````


````warn header="`try...catch` працює синхронно"
Якщо помилка трапляється у "запланованому до виконання" коді, як от в `setTimeout`, тоді `try...catch` не зможе перехопити помилку:
...
Щоб перехопити помилку всередині функції запланованої до виконання, `try...catch` повинен бути всередині тієї функції:
```js run
setTimeout(function() {
  try {
...
В такому разі `JSON.parse` спрацює нормально і блок `catch` буде проігнорованим. Але з нашої точки зору в нас є помилка - відсутність поля `name`.
...
Наприклад, ми хочемо виміряти час роботи функції, що рахує числа Фібоначчі. Для цього ми можемо почати вимірювання на початку виконання і закінчити після. А якщо протягом роботи функції виникне помилка? Зокрема, імплементація `fib(n)` нижче генерує помилки, якщо на вхід подано від’ємне або неціле число.
