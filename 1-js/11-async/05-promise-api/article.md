...
Уявімо ми хочемо запустити кілька промісів одночасно і дочекатись поки всі вони виконаються.
...
`Promise.all` приймає ітеровуваний об’єкт (зазвичай масив промісів) і повертає новий проміс.
...
Перший проміс тут був найшвидшим, але завершився з помилкою, тож результатом став другий проміс. Після того, як перший завершений проміс «перемагає», усі подальші результати ігноруються.
...
1. `Promise.all(promises)` -- чекає виконання всіх промісів і повертає масив їх результатів. Якщо хоча б один з наданих промісів завершується з помилкою, вона стає загальною для `Promise.all`, а всі інші результати ігноруються.
...
5. `Promise.resolve(value)` -- повертає успішно завершений проміс із заданим значенням.
