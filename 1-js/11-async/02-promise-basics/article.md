...
2. Є код "споживач", котрий очікує отримати результат від коду "виконавця", як тільки той буде готовий. Він може бути потрібним багатьом функція. Це -- "фанати"
3. *Promise* (надалі будемо називати такий об'єкт проміс) -- це спеціальний об'єкт в JavaScript, котрий зв'язує код "виробник" і "споживач" разом. В контексті нашої аналогії -- це "список для підписки". Код "виробник" реалізовується стільки часу, скільки потрібно щоб отримати результат, а проміс, як тільки результат готовий, робить його доступним для коду який підписався на конкретний проміс.
...
Нижче приклад конструктора проміса і простої функція виконавця з кодом "виробником", що видає результат з затримкою (через `setTimeout`):
...
2. Виконавець отримує два аргументи: `resolve` і `reject` -- ці функція вмонтовані в JavaScript, тому нам непотрібно їх створювати. Нам слід всього лиш викликати одну з них по готовності.
