...
Опціональний ланцюжок `?.` -- це безпечний спосіб доступу до властивостей об'єктів з вкладеностями, навіть якщо проміжних властивостей не існує.
...
Отож якщо користувач не вказав адресу, а ми своєю чергою намагаємось отримати доступ до властивості `user.address.street`, то отримаємо помилку.
...
Ще один приклад. У веб розробці ми можемо отримати об'єкт котрий відповідає елементу на веб сторінці за допомогою спеціальних методів, наприклад: `document.querySelector('.elem')`. Проте якщо ми намагатимемось отримати елемент якого немає на сторінці, то нам вернеться `null`.
...
Для закріплення. Якщо елемент не існує на веб сторінці, ми отримаємо помилку при спробі доступитись до властивості `.innerHTML` оскільки при спробі його знайти ми отримаємо `null`. І в деяких випадках, коли відсутність елемента для нас є нормою, ми хотіли б просто отримати `.innerHTML = null` (тобто `html = null`).
...
Як вже говорилось, `?.` негайно припиняє ("скорочене обчислення") обчислення, якщо лівої частини не існує.
