...
Як видно з коду, об’єкт `user` може перетворюватися в рядок, який містить ім’я користувача, або в число, що дорівнює сумі його коштів. Єдиний метод `[Symbol.toPrimitive]` об’єкту `user` обробляє всі випадки перетворення, в залежності від типу.
...
Методи `toString` і `valueOf` походять з давніх часів. Це не методи з символьними ключами (колись символів не існувало), а "звичайні" методи з рядковими ключами. Вони надають альтернативний «старомодний» спосіб реалізації перетворення.
...
Немає контролю, чи повертає `toString` саме рядок, або чи метод `Symbol.toPrimitive` повертає число для підказки `"number"`.
