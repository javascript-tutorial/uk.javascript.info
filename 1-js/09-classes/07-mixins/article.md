...
В JavaScript ми можемо лише успадкувати від одного об'єкта. Може бути лише один `[[Prototype]]` для об'єкта. І клас може розширювати лише один інший клас.

Але іноді це викликає обмеження. Наприклад, у нас є клас `StreetSweeper` і клас `Bicycle`, і хочемо зробити їхню суміш: `StreetSweepingBicycle`.
...
Іншими словами, *mixin* забезпечує методи, які реалізують певну поведінку, але ми не використовуємо його самостійно, ми використовуємо його, щоб додати поведінку до інших класів.
...
Наприклад, тут використовується міксин `sayHiMixin`, щоб додати можливість мовити для `User`:
...
Зверніть увагу, що заклик до батьківського методу `super.say()` з `sayHiMixin` (на лініях, позначених `(*)`), шукає метод у прототипі цього міксину, а не класу.

Ось діаграма (див. праву частину):

![](mixin-inheritance.svg)

Це тому, що методи `sayHi` і `sayBye` були спочатку створені в `sayHiMixin`. Таким чином, хоча вони були скопійовані, їх внутрішня власність `[[HomeObject]]` посилається на `sayHiMixin`, як показано на малюнку вище.
...
Тепер, якщо ми хотіли б, щоб будь-який код реагував на вибір меню, то ми можемо слухати цю подію за допомогою `menu.on(...)`.
