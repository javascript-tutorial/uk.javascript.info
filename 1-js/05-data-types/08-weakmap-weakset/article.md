...
// ми можемо отримати його завдяки using map.keys()
*/!*
```
...
weakMap.set(obj, "ок"); // працює добре (об’єкт є ключем)
...
Якщо ми працюємо з об’єктом, що "належить" до іншого коду, можливо навіть сторонньої бібліотеки, і хотіли б зберегти деякі дані, пов’язані з ним, що повинні існувати лише поки об’єкт живий -- тоді `WeakMap` є саме те, що потрібно.

Ми покладемо ці дані в `WeakMap`, використовуючи об’єкт як ключ, і коли об’єкт буде видалено збирачем сміття, то також дані автоматично зникнуть.
