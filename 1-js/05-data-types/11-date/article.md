...
Познайоммось з новий вбудованим об’єктом: [Date](mdn:js/Date). Він зберігає дату, час і надає методи для управління датою/часом.
...
: Створює об’єкт `Date` з часом, що дорівнює кількості мілісекунд (1/1000 секунди), що минули після 1 січня 1970 UTC+0.
    ...
    Ціле число, яке являє собою кількість мілісекунд, що пройшли з початку 1970 року, називається *timestamp*.
...
: Повертає timestamp для дату -- кількість мілісекунд, що пройшли з 1 січня 1970 UTC+0.
...
```js
// У нас є date1 і date2, яка функція швидше повертає їхню різницю в мс?
...
Ці дві функції роблять саме те ж саме, але одна з них використовує явний `date.getTime()`, щоб отримати дату в мс, а інша покладається на перетворення дати до числа. Їх результат завжди однаковий.

Отже, яка з них швидше?
...
Сучасні рушії JavaScript виконують багато оптимізацій. Вони можуть налаштувати результати "штучних тестів" у порівнянні з "нормальним використанням", особливо коли ми тести щось дуже мале, наприклад, як працює оператор, або вбудована функція. Отже, якщо ви серйозно хочете оцінити продуктивність, то, будь ласка, дослідіть, як працює рушій JavaScript. І тоді ви, мабуть, не потребуєте мікробенчмаркінгів взагалі.
