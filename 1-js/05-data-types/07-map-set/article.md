...
Використання об'єктів як ключів -- це одна з відомих і часто застосовуваних можливостей колекції Map. Ми можемо використати ключі-рядки для об'єкта `Object`, але для ключів-об'єктів -- вже ні.
...
Оскільки `visitsCountObj` -- це об'єкт, він конвертує всі ключі, такі як `john` і `ben`, до рядка `"[object Object]"`. Це однозначно не той результат, який ми очікуємо.

```smart header="Як об'єкт `Map` порівнює ключі"
