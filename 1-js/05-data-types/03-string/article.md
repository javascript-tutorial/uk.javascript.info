...
В прикладі вище `alert` не відображається, оскільки `str.indexOf("Widget")` повертає `0` (це означає, що він знайшов збіг у початковій позиції). Це правильно, але `if` вважає, що `0` -- це `false`.
...
alert( "Widget".includes("id", 3) ); // false, починаючи з 3-го символа, підрядка "id" немає
```
