...
- Потім подія рухається вниз від кореня документа до `event.target`, викликаючи на шляху обробники, призначені з використанням `addEventListener(..., true)` (значення `true` це скорочення `{capture: true}`).
