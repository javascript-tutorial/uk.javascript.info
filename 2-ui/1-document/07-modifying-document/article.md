...
Можна створити функцію та помістити код в неї. Але є ще один спосіб -- *клонувати* існуючий `div` та змінити текст всередині (якщо потрібно).
...
Ці методи прийшли з давніх часів. Сьогодні немає причин їх використовувати, оскільки сучасні методи, такі як `append`, `prepend`, `before`, `after`, `remove`, `replaceWith` набагато зручніші у використанні.
...
Тому він працює надзвичайно швидко, адже не відбувається *жодних DOM-модифікацій*. Він записує беспосередньо в текст сторінки, поки DOM ще не сформований.
